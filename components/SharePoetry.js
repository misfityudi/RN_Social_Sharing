...
const ShareGIF = (props) => {
    ...
    const [poetry, setPoetry] = useState('This is test poetry');
    ...
    const fetchSharingDetails = async () => {
        ...
        await fetch(api_url, {signal:signal})
            ...
            .then((response) => {
                setImageURL(response.details.image_url);
                setPoetry(response.poetry);
            })
    ...
    return(
        <TouchableOpacity onPress={shareHandler} style={styles.container}>
            <Text style={styles.text}>Share Poetry</Text>
        </TouchableOpacity>
    )
}
